            ...
            <div className="pt-4">
                <div className="mb-6 flex-shrink-0">
                    <div className={`flex items-center mb-4 ${isCalendarCollapsed ? 'justify-center' : 'justify-between'}`}>
                        {!isCalendarCollapsed && <button onClick={handlePrevMonth} className="p-1 text-[var(--color-text-secondary)] hover:text-[var(--color-text-primary)]" aria-label="Previous month"><ChevronLeftIcon /></button>}
                        <button 
                        ...
                        {!isCalendarCollapsed && <button onClick={handleNextMonth} className="p-1 text-[var(--color-text-secondary)] hover:text-[var(--color-text-primary)]" aria-label="Next month"><ChevronRightIcon /></button>}
                    </div>
                    <div className={`transition-all duration-500 ease-in-out overflow-hidden ${isCalendarCollapsed ? 'max-h-32' : 'max-h-[30rem]'}`}>
                        <div className="grid grid-cols-7 gap-1 text-center">
                            {dayHeaders.map(day => <div key={day} className="text-sm h-8 flex items-center justify-center font-medium text-[var(--color-text-secondary)]">{day}</div>)}
                                ...
                                return (
                                    <div key={index} className="flex justify-center items-start">
                                        {dayObj.day && (
                                            ...
                                            >
                                                <span className={`w-6 h-6 flex items-center justify-center rounded-full ${isSelected ? '' : isToday ? 'bg-[var(--color-surface-container)]' : ''}`}>
                                                    {dayObj.day}
                                                </span>
                                                 {tagsForDay.length > 0 && (
                ...
                <div className="flex-grow flex flex-col">
                    ...
                    {selectedDayMoments.length === 0 ? (
                        <div className="text-center py-8 text-[var(--color-text-secondary)]">No check-ins for this day.</div>
                    ) : (
