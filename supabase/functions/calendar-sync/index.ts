...
serve(async (req)=>{
  ...
  try {
    ...
    const { data: connection, error: connError } = await supabaseAdmin.from('calendar_connections').select('refresh_token').eq('user_id', user.id).eq('provider', 'google').single();
    ...
    const { data: tasksToSync, error: tasksError } = await supabaseAdmin.from('tasks').select('*').eq('user_id', user.id).neq('dueDate', null);
    ...
    for (const task of tasksToSync){
      ...
      try {
        ...
        if (!eventId) {
          ...
          const { error: updateDbError } = await supabaseAdmin.from('tasks').update({
