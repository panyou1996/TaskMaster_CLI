...
// FIX: Using the official esm.sh path for Supabase functions types to resolve Deno and other runtime types.
...
serve(async (req) => {
  ...
  // Construct the redirect URL for the frontend app
  const appUrl = Deno.env.get('SUPABASE_URL')?.replace('supabase.co', 'vercel.app') || Deno.env.get('SUPABASE_URL');
  ...
  try {
    ...
    const { error: upsertError } = await supabaseAdmin.from('calendar_connections').upsert(
      connectionData, 
