                ...
                <div ref={cardRef} className="bg-white rounded-xl card-shadow p-4">
                    ...
                    <div className="flex items-start gap-3">
                        ...
                        <div className="flex-grow">
                            <input type="text" value={title} onChange={e => setTitle(e.target.value)} onFocus={() => setActiveInput('title')} placeholder="New To-Do" className="w-full text-base font-semibold text-gray-900 placeholder-gray-800 focus:outline-none bg-transparent" />
                            <textarea value={notes} onChange={e => setNotes(e.target.value)} onFocus={() => setActiveInput('notes')} placeholder="Notes" rows={1} className="w-full text-sm mt-1 text-gray-700 placeholder-gray-500 focus:outline-none resize-none bg-transparent" />
                            {isSubtaskSectionVisible && (
                                <div className="mt-2 space-y-1 pt-2 border-t border-gray-100">
                                    {subtasks.map((sub) => (
                                        <div key={sub.id} className="flex items-center gap-2 group">
                                            <SubtaskCircleIcon />
                                            <input type="text" value={sub.text} onChange={e => { const newText = e.target.value; setSubtasks(subs => subs.map(s => s.id === sub.id ? { ...s, text: newText } : s)); }} className="w-full text-sm text-gray-800 focus:outline-none bg-transparent" />
                        ...
                        <div className="flex items-center justify-end">
                            <div className="flex items-center gap-1">
                                ...
                                <div ref={calendarIconRef} className="relative">
                                    ...
                                    {activePopover === 'dueDate' && (<div className="absolute w-56 bg-white rounded-lg shadow-lg p-3 z-10 animate-page-fade-in" style={popoverPosition}><div><label className="text-xs font-medium text-gray-500">Due Date</label><input type="date" value={dueDate} onChange={e => { setDueDate(e.target.value); }} className="w-full mt-1 p-2 border border-gray-200 rounded-md text-sm focus:outline-none focus:ring-1 focus:ring-blue-500"/></div></div>)}
                                </div>
                                <div ref={listIconRef} className="relative">
                                    ...
                                    {activePopover === 'list' && (<div className="absolute w-48 bg-white rounded-lg shadow-lg p-2 z-10 animate-page-fade-in max-h-48 overflow-y-auto" style={popoverPosition}>{listOptions.map(listName => (<button key={listName} type="button" onClick={() => { setCategory(listName); setActivePopover(null); }} className={`w-full text-left px-3 py-2 text-sm rounded-md flex justify-between items-center ${category === listName ? 'bg-blue-100 text-blue-700' : 'hover:bg-gray-100'}`}><span>{listName}</span>{category === listName && <CheckIcon className="w-4 h-4" />}</button>))}</div>)}
                                </div>
                                ...
                                <div ref={reminderIconRef} className="relative">
                                    <button type="button" title="Set Reminder" onClick={() => handlePopoverToggle('reminder', reminderIconRef)} className={`p-2 rounded-full transition-colors ${reminder !== null ? 'text-blue-600 bg-blue-100' : 'text-gray-500 hover:bg-gray-100'}`}><BellIcon className="w-5 h-5" /></button>
                                    {activePopover === 'reminder' && (<div className="absolute w-48 bg-white rounded-lg shadow-lg p-2 z-10 animate-page-fade-in max-h-48 overflow-y-auto" style={popoverPosition}>{reminderOptions.map(option => (<button key={option.label} type="button" onClick={() => { setReminder(option.value); setActivePopover(null); }} className={`w-full text-left px-3 py-2 text-sm rounded-md flex justify-between items-center ${reminder === option.value ? 'bg-blue-100 text-blue-700' : 'hover:bg-gray-100'}`}><span>{option.label}</span>{reminder === option.value && <CheckIcon className="w-4 h-4" />}</button>))}</div>)}
                                </div>
